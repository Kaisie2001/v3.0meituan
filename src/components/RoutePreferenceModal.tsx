...
const transportOptions: Array<{ label: string; value: TransportMode; desc: string }> = [
  { label: "公共交通", value: "transit", desc: "更符合城市出行" },
  { label: "私家车", value: "driving", desc: "更快但停车不确定" },
  { label: "步行", value: "walking", desc: "更慢但更自由" },
];
...
const goalOptions: Array<{ label: string; value: OptimizeGoal; desc: string }> = [
  ...
  { label: "其他", value: "custom", desc: "用语音或文字说明偏好" },
];
    ...
    <div className="pointer-events-auto absolute inset-0 z-[3000] flex items-end justify-center bg-black/35">
      ...
      >
        ...
        <div className="shrink-0 px-4 pb-2 pt-3">
          ...
          <p className="mt-1 text-sm leading-5 text-black/60">选择出行方式和路线目标，系统会据此重新规划。</p>
        </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {transportOptions.map((item) => (
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-xs text-black/55">{item.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-xl bg-meituan-gray p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {goalOptions.map((item) => (
                  ...
                  >
                    <p className="text-sm font-extrabold text-black/80">{item.label}</p>
                    <p className="mt-0.5 text-[11px] leading-4 text-black/55">{item.desc}</p>
                  </button>
                ))}
              </div>
              ...
              {goal === "custom" ? (
                <div className="mt-3 rounded-xl border border-black/10 bg-white p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="text-sm font-extrabold text-black/75">语音或文字输入</p>
                    <button
                      type="button"
